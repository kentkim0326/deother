...
const P_LAYOUT = {
  ...
  festivals: {
    ...
    after: {
      0: [{ s: 6 }],                          // 지금까지 넣은 것 뒤   → 타이베이 IGA 신청 화면
      2: [{ s: 3 }],                          // 2026 접수(15항목) 뒤  → FN 아레나 아트
      3: [{ s: 1 }],                          // 2026 개최 뒤         → 우리 부스 간판
      5: [{ s: 4 }],                          // 2027 4~6월(16항목) 뒤 → FN 국가별 파이터
      6: [{ s: 2 }],                          // 2027 개최 일정 뒤     → 배급사 미팅
      8: [{ s: 5 }],                          // 어떤 순서로 도전하나 뒤 → 킹덤워즈(온라인 트랙)
    },
  },
  kwonline: {
