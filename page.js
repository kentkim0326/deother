...
const P_LAYOUT = {
  ...
  deathguide: {
    ...
    // 섹션이 4개뿐이라 구간마다 두 장씩 깔아 리듬을 준다. 여기도 자막 배열이 없어 {img} 다.
    // ⚠️ 성인용(극심한 고어) 표기는 본문에 그대로 있다 — 이미지를 앞으로 끌어냈다고 완화하지 않는다.
    top: [{ img: 0 }],                        // 인트로 아래 → 키비주얼 커버
    after: {
      0: [{ img: 1 }, { img: 2 }],            // 저승사자 백무진 뒤
      1: [{ img: 3 }, { img: 4 }],            // 베고, 절단한다 뒤
      2: [{ img: 5 }, { img: 6 }],            // 끊임없는 망령 뒤
      3: [{ img: 7 }, { img: 8 }],            // 지금 어디까지 뒤
    },
  },
};
