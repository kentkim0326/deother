...
const P_LAYOUT = {
  miliverse: {
    ...
    after: {
      // 2026-08-14: 실제 게임 화면이 들어왔다(D:\Miliverse). 예전엔 콘셉트 아트뿐이라
      // "공공데이터를 게임에 넣었다"를 글로만 주장했는데, 이제 주장하는 자리마다 화면이 붙는다.
      // 아트는 감정을 만드는 자리(0·5)에만 남기고, 데이터·영토를 주장하는 자리는 실제 화면으로 바꿨다.
      // 자막이 붙는 건 {s:5}(리더보드)·{s:7}(접속 보상) 둘뿐 — i18n.js 의 MV_SLIDES 주석 참고.
      0: [{ img: 0 }],                        // 왜 만들었나   → WWII 사단(석양). 아트로 연다
      1: [{ img: 8 }],                        // 전쟁          → 3D 도시 지도 (실제 화면)
      2: [{ v: 1 }, { img: 6 }],              // 실시간 데이터 → 2분 소개 영상 + 도쿄 타일 격자
      3: [{ img: 9 }],                        // 무기/배치     → '내 땅' 실제 좌표 + 배치된 무기
      4: [{ s: 5 }],                          // 하는 방법     → 글로벌 리더보드 (자막 있음)
      5: [{ img: 4 }, { s: 7 }, { v: 2 }],    // AI/데이터     → 아트 + 접속 보상(자막) + v2.4 쇼츠
    },
  },
  jangbogo: {
  ...
  festivals: {
    ...
    after: {
      ...
      // '받은 것' 은 이 페이지에서 증거가 가장 필요한 자리다. 2026-08-18 에 인천국제공항공사
      // 상생협력 업무협약식 사진 두 장을 앞에 붙였다 — 최우수상이 협약으로 이어진 것이라 여기가 맞다.
      1: [{ s: 12 }, { s: 13 }, { s: 9 }],     // 지금까지 받은 것 뒤 → 협약식 단체·대표, 그 다음 GGC 배너
