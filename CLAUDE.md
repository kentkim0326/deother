...
**3. 진입 동선은 여정 연표 바로 아래 링크(`journey.more`, 10개 언어)다.**
`index.html #jrMore` + `app.js` + `i18n.js` 세 곳. 본문은 `pages-i18n.js` 의 `festivals`
블록이고 **우선 ko/en** 이다(다른 언어는 자동으로 en 으로 떨어진다).
이미지·영상이 없어 `P_LAYOUT` 항목도 없다 — 폴백 경로로 글만 렌더된다.
