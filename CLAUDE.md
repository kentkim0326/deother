...
- **인터리브** (밀리버스 · 장보고 · KWO · 타임워즈 · 터프킹): 본문 섹션 사이사이에 미디어를 끼워 넣는다.
  `{v:n}` = 영상, `{s:n}` = 자막 있는 스크린샷, `{img:n}` = 자막 없는 시네마틱 밴드,
  `{roster:[i,j,…]}` = 세로 캐릭터 여러 명을 한 줄에 나란히(터프킹 로스터). n·인덱스는 `pSlideSet.files`.
...
- [ ] **숨바·저승사자·타임워즈·터프킹·월스트리트 상세 10개 언어 확장** (지금 ko/en). 카드는 이미 10개 언어다.
