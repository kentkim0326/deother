...
본문은 `pages-i18n.js` 의 `festivals` 블록, **우선 ko/en**(다른 언어는 en 으로 떨어진다).
...
고칠 때 ko/en **양쪽을 같이** 고치고, 마지막 섹션의 "마지막 갱신" 날짜도 함께 올린다.
...
- **연도별 행사 로드맵(`festivals.html`) 추가**(2026-08-16) — 2026 하반기~2027 도전 행사 38종을 달별로.
  ko/en, 여정 연표 아래 `journey.more` 링크(10개 언어). 위 「⚠️ 연도별 행사 로드맵」 필독.
...
- [ ] **행사 로드맵 8개 언어 확장** (지금 ko/en). 다른 상세와 같은 상태다.
      ※ 카드가 아니라 상세 본문 + 사진 자막(`festivals.slides`) 둘 다 옮겨야 한다.
