...
- 책: 문서가 21권(제목)과 22권(SF 슬라이드)으로 갈리고 `i18n.js` 의 team 은 20권이다.
  사이트 안에서 서로 어긋나면 안 되니 **"스무 권이 넘는"** 으로 썼다. 숫자가 확정되면 바꾼다.
...
- **뺐다: "데니스 리치 교수 밑에서 컴퓨터 공학 수업"** — 리치는 하버드 교수가 아니라
  벨연구소 연구원이었다. 확인 전에는 쓰지 않는다. 지금은 "컴퓨터 공학 수업을 들었다"까지만.
...
※ 신동아 지면 스캔은 제3자 저작물이다. 대표님 본인 기사지만 **문제 되면 빼거나 링크로 바꾼다.**
...
- [ ] **숨바·저승사자·타임워즈·터프킹·월스트리트·대표 소개 상세 10개 언어 확장** (지금 ko/en).
      카드는 이미 10개 언어다.
- [ ] **대표 소개의 숫자 확정** — 책이 20/21/22권으로 갈린다. 대표님이 정해주면 `founder` 블록과
      `i18n.js` 의 team bullets 를 같은 숫자로 맞춘다. 하버드 입학 연차(3년 vs 3년 반)도 마찬가지.
